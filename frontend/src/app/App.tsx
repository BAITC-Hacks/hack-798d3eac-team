...
function Shell({ children, theme, onToggleTheme }: { children: React.ReactNode; theme: 'light' | 'dark'; onToggleTheme: () => void }) {
  return <div className="app"><header className="topbar"><Link to="/" className="brand"><span className="brand-mark">S</span><span>AI Sana<small>Практические задачи</small></span></Link><nav><NavLink to="/tasks"><ClipboardList size={17}/> Каталог</NavLink><NavLink to="/business"><BriefcaseBusiness size={17}/> Бизнес</NavLink><NavLink to="/teams"><Users size={17}/> Команды</NavLink></nav><span className="connected"><i/> Django API</span><button className="theme-toggle" onClick={onToggleTheme} aria-label={theme === 'light' ? 'Включить тёмную тему' : 'Включить светлую тему'} title={theme === 'light' ? 'Тёмная тема' : 'Светлая тема'}>{theme === 'light' ? <Moon size={17}/> : <Sun size={17}/>}<span>{theme === 'light' ? 'Тема' : 'Тема'}</span></button></header><main>{children}</main><footer><span>AI Sana</span><span>Практический хакатон · Задачи, которые двигают вперёд</span></footer></div>;
}
...
export default function App() {
  const [theme, setTheme] = useState<'light'|'dark'>(() => localStorage.getItem('ai-sana-theme') === 'dark' ? 'dark' : 'light');
  useEffect(() => { document.documentElement.dataset.theme = theme; localStorage.setItem('ai-sana-theme', theme); }, [theme]);
