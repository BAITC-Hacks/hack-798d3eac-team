...
import { emptyTask, ProposalModel, type Task, type TaskAnalysis, type TaskInput, type TeamInput } from '../domain/models';
...
function Level({ level }: { level: Task['readiness_level'] }) { const label = ({ draft: 'Нужно уточнение', working: 'Рабочая', ready: 'Готовая', priority: 'Приоритетная' } as const)[level]; return <span className={`level level-${level}`}>{label}</span>; }
function shortReason(reasons: string[]) {
  return reasons[0]?.replace(/\s*\(критерий заполнен.*$/, '').replace(/[.!?]+$/, '') ?? 'Ключевые критерии раскрыты';
}
function RatingSummary({ score, level, reasons }: { score: number; level: Task['readiness_level']; reasons: string[] }) {
  return <div className="rating-summary"><div className="rating-value"><b>{score}</b><span>/ 100</span><div className="meter"><i style={{ width: `${Math.max(0, Math.min(100, score))}%` }}/></div></div><div className="rating-details"><Level level={level}/><span className="rating-reason">{shortReason(reasons)}</span></div></div>;
}
function Score({ task }: { task: Task }) { return <RatingSummary score={task.score} level={task.readiness_level} reasons={task.missing_information}/>; }
