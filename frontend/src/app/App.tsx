...
function Shell({ children }: { children: React.ReactNode }) {
  return <div className="app"><header className="topbar"><Link to="/" className="brand"><span className="brand-mark">S</span><span>AI Sana<small>Практические задачи</small></span></Link><nav><NavLink to="/tasks"><ClipboardList size={17}/> Каталог</NavLink><NavLink to="/business"><BriefcaseBusiness size={17}/> Бизнес</NavLink><NavLink to="/teams"><Users size={17}/> Команды</NavLink></nav><span className="connected"><i/> Django API</span></header><main>{children}</main></div>;
}
...
function Loading() { return <div className="loading"><span className="spinner"/>Загружаем данные из Django…</div>; }
...
function TaskGrid({ tasks }: { tasks: Task[] }) {
  ...
  return <div className="task-grid">{tasks.map(t => <article className="task-card" key={t.id}><div className="card-top"><span className="category">{t.industry || 'Без отрасли'}</span><Level task={t}/></div><h3>{t.title || `Бизнес-задача #${t.id}`}</h3><p>{t.need || t.context || 'Описание задачи будет добавлено бизнесом.'}</p><div className="card-footer"><Score task={t}/><Link className="button outline small" to={`/tasks/${t.id}`}>Подробнее <span>→</span></Link></div></article>)}</div>;
}

function Catalog() {
  const [industry, setIndustry] = useState('');
  const { value: tasks, busy, error, reload } = useLoad(() => taskApi.list(industry || undefined), [industry]);
  return <><PageTitle eyebrow="ДЛЯ СТУДЕНЧЕСКИХ КОМАНД" title="Каталог задач" text="Все опубликованные задачи открыты для отклика — даже если им пока нужно уточнение."/><div className="toolbar"><label>Отрасль<input value={industry} onChange={e => setIndustry(e.target.value)} placeholder="Например, торговля"/></label><span className="sort-note">Сортировка по готовности: сначала наиболее полные</span></div>{busy ? <Loading/> : error ? <Notice error={error} retry={reload}/> : <TaskGrid tasks={tasks ?? []}/>}</>;
}
...
function TaskDetail({ id }: { id: number }) {
  const { value: task, busy, error, reload } = useLoad(() => taskApi.get(id), [id]);
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setSending(true); setFormError(''); setMessage(''); try { await proposalApi.create({ task_id:id, team_id:Number(teamId), idea, plan, duration, prototype_url:url }); setMessage('Отклик отправлен представителю бизнеса.'); setIdea(''); setPlan(''); setDuration(''); setUrl(''); } catch (x) { setFormError(x instanceof Error ? x.message : 'Не удалось отправить отклик'); } finally { setSending(false); } };
  if (busy) return <Loading/>; if (error) return <Notice error={error} retry={reload}/>; if (!task) return null;
  return <><Link className="back-link" to="/tasks">← Назад в каталог</Link><div className="detail-head"><div><div className="eyebrow">{task.industry || 'БИЗНЕС-ЗАДАЧА'}</div><h1>{task.title || `Задача #${task.id}`}</h1><p>{task.need}</p></div><div className="detail-score"><Score task={task}/><Level task={task}/></div></div><div className="detail-grid"><section className="panel"><h2>Описание задачи</h2>{fields.filter(f => f.name !== 'title' && f.name !== 'industry').map(f => <div className="detail-field" key={f.name}><b>{f.label}</b><p>{task[f.name] || <span className="muted">Пока не указано</span>}</p></div>)}</section><form className="panel proposal-form" onSubmit={submit}><div className="step-label">ПРЕДЛОЖЕНИЕ КОМАНДЫ</div><h2>Предложить решение</h2><label className="field"><span>Команда *</span><select required value={teamId} onChange={e => setTeamId(e.target.value)}><option value="">Выберите команду</option>{(teams ?? []).map(team => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label><label className="field"><span>Идея решения *</span><textarea required rows={3} value={idea} onChange={e => setIdea(e.target.value)}/></label><label className="field"><span>План работы *</span><textarea required rows={3} value={plan} onChange={e => setPlan(e.target.value)}/></label><label className="field"><span>Срок *</span><input required value={duration} onChange={e => setDuration(e.target.value)} placeholder="Например, 3 недели"/></label><label className="field"><span>Ссылка на прототип</span><input type="url" value={url} onChange={e => setUrl(e.target.value)} placeholder="https://"/></label>{formError && <Notice error={formError}/ >}{message && <div className="success">{message}</div>}<button className="button primary full" disabled={sending || !teams?.length}>{sending ? 'Отправляем…' : 'Отправить отклик'}</button>{!teams?.length && <p className="muted">В бэкенде пока нет команд. Создайте команду через Django admin.</p>}</form></div></>;
}

function TaskRoute() { const id = Number(window.location.pathname.split('/').filter(Boolean).at(-1)); return <TaskDetail id={id}/>; }

function TeamPage() {
  const { value: teams, busy, error, reload } = useLoad(() => teamApi.list());
  return <><PageTitle eyebrow="УЧАСТНИКИ" title="Команды" text="Демо-профили команд загружаются напрямую из Django API."/>{busy ? <Loading/> : error ? <Notice error={error} retry={reload}/> : <div className="team-grid">{(teams ?? []).map(t => <article className="panel team-card" key={t.id}><div className="team-avatar">{t.name.slice(0,1).toUpperCase()}</div><h3>{t.name}</h3><p><b>Интересы</b>{t.interests || 'Не заполнены'}</p><p><b>Навыки</b>{t.skills || 'Не заполнены'}</p><p><b>Технологии</b>{t.technologies || 'Не заполнены'}</p></article>)}</div>}</>;
}
...
export default function App() {
  return <Shell><Routes><Route path="/" element={<Home/>}/><Route path="/tasks" element={<Catalog/>}/><Route path="/tasks/:id" element={<TaskRoute/>}/><Route path="/business" element={<Business/>}/><Route path="/business/new" element={<Business/>}/><Route path="/proposals" element={<Proposals/>}/><Route path="/teams" element={<TeamPage/>}/><Route path="*" element={<div className="empty">Страница не найдена · <Link to="/">На главную</Link></div>}/></Routes><footer>AI Sana · Практический хакатон <span>Фронтенд подключён к Django REST endpoints</span></footer></Shell>;
}
